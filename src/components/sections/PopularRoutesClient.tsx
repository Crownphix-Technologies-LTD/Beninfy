...
export default function PopularRoutesClient({ routes }: { routes: PopularRouteCard[] }) {
        ...
        {routes.map((route) => (
          ...
          >
            <div className="h-48 overflow-hidden relative">
              <Image
                src={route.image || ROUTE_IMAGES[route.id] || ROUTE_IMAGES['lagos-cotonou']}
              ...
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-label-sm text-primary">
                {ROUTE_BADGES[route.id] ?? 'Private Route'}
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-secondary text-headline-sm">
                  {route.startingPriceNGN ? formatNGN(route.startingPriceNGN) : 'Quote'}
                </span>
              </div>
